# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

**ryder** is a Hugo theme (v0.1) built with TailwindCSS, Alpine.js, and Font Awesome. Named after a late Rhodesian Ridgeback/Mastiff companion, it's intentionally barebones with practical additions for quick setup.

- **Live Demo:** https://arts-link.github.io/ryder/
- **License:** MIT
- **Production Sites:** benstrawbridge.com, arts-link.com, writingsos.com, grrquarterly.com

## Technology Requirements

- **Hugo:** 0.146+ (compatibility requirement)
- **Node.js/npm:** For dependency management
- **TailwindCSS:** v3.4.0+
- **Alpine.js:** Lightweight JavaScript framework
- **Font Awesome:** v6.5.1+

## Development Commands

### CSS Development

There is no separate CSS build step. Tailwind compiles inside the Hugo build via
`head/css.html`'s `css.PostCSS` and `postcss.config.js`, so `hugo server` and
`hugo --minify` are the whole workflow. The `build-tw` / `deploy-tw` /
`watch-tw` scripts were removed in v0.3.0 — they wrote a `style.css` no
template read.

### Hugo Commands
```bash
hugo server -D                    # Local development with drafts
hugo server -D --source exampleSite  # Test theme in exampleSite
hugo --minify                     # Production build
```

### Testing
The `exampleSite/` directory is the primary test environment. All theme changes MUST be tested here before release.

Required verification before push:
- Run `npm test`
- Run `hugo --source exampleSite --destination /tmp/ryder-check`
- Do not rely only on a targeted test when the change touched shared templates, CSS, JS, or theme configuration

## Release Maintenance

- When the theme version receives a minor version bump or anything greater, refresh the Hugo themes submission screenshots in `images/screenshot.png` (1500x1000) and `images/tn.png` (900x600).
- `exampleSite/hugo_stats.json` is generated, but it is intentionally tracked because Tailwind clean builds depend on it. Do not remove it from git. Regenerate and commit it when template, content, or config changes affect Tailwind class discovery.

## Architecture Overview

### Template Hierarchy & Customization

The theme uses Hugo's template lookup order with a powerful customization system:

1. **Base Templates** (`layouts/_default/`)
   - `baseof.html` - Root template with header/footer partials
   - `home.html`, `list.html`, `single.html` - Page type templates
   - `hidden-home/baseof.html` - Special cover image template

2. **Dynamic Partial Loading** (`layouts/partials/`)
   Theme supports variants via configuration parameters:
   - Header: `header.html` (default) or a site override like `header-custom.html` via `headerType = "-custom"`
   - Menu: `menu.html` (default) or a site override like `menu-custom.html` via `menuType = "-custom"`
   - Footer: `footer.html` (default) or a site override like `footer-custom.html` via `footerType = "-custom"`
   - Cards: Multiple variants (`card-super-simple.html`, etc.) via `listCardType`

   **Implementation pattern in baseof.html:**
   ```go
   {{ $headerType := .Param "headerType" | default "" }}
   {{- partial (printf "header%s.html" $headerType) . }}
   ```
   This allows users to set custom variants in config or front matter without editing theme files.

3. **Partial Organization**
   - `partials/common-partials/` - SEO, OpenGraph, Twitter Cards
   - `partials/head/` - CSS, JS, fonts, favicon, schema markup
   - `partials/utils/` - Reusable utilities (dark toggle, page meta, social lists)

4. **Shortcodes** (`layouts/shortcodes/`)
   Content embeds: alerts, photo galleries, affiliate links, maps (Leaflet/OpenStreetMap), recipe schemas, custom media players (Soundcloud, Webamp)

5. **Custom Content Types**
   - `photo-gallery/` - `layouts/photo-gallery/single.html` renders photo gallery pages as a distinct layout

6. **Image Render Hook**
   - `layouts/_default/_markup/render-image.html` - Custom render hook wraps all Markdown images for responsive/lazy loading

### JavaScript Architecture

- **Entry point:** `assets/js/main.js` — bootstraps Font Awesome SVG replacement, Alpine.js, and conditionally loads Leaflet and the dark mode module based on Hugo-injected JS variables (`loadLeaflet`, `showDarkToggle`)
- **`assets/js/themeSwitcher.js`** — dark mode logic, imported dynamically
- **`assets/js/backgroundChanger.js`** — random background image utility for `hidden-home` layout
- **`assets/js/extended.js`** — intentionally empty; meant to be overridden by users in their site's `assets/js/extended.js` to add custom JS without modifying theme files

### Adding Font Awesome Icons

The theme uses tree-shaken FA imports — only explicitly imported icons are included in the JS bundle. If an icon class is used in content or templates but not imported, Font Awesome renders a warning placeholder icon instead.

**To add an icon:**
1. Find the camelCase name: `fa-bowl-food` → `faBowlFood`
2. Add the import to `assets/js/main.js` (keep list alphabetical):
   ```js
   import { ..., faBowlFood } from '@fortawesome/free-solid-svg-icons'
   ```
3. Add it to `library.add(...)` in the same file

Solid icons come from `@fortawesome/free-solid-svg-icons`, brand icons from `@fortawesome/free-brands-svg-icons`, regular (outline) from `@fortawesome/free-regular-svg-icons`. Check [fontawesome.com/icons](https://fontawesome.com/icons) (filter: Free) to confirm availability before adding.

### Data Files

- **`data/social.json`** — Social icon links displayed in the footer. Structure: `{ "main": [{ "title", "name", "icon", "link", "weight" }] }`.
- **`data/theme.json`** — Color palette and font family definitions (used for reference/documentation; not dynamically consumed by templates).

### Static Assets

- **`static/leaflet/`** — Leaflet.js is bundled locally (not loaded from CDN) to avoid external requests. Only loaded when `loadLeaflet = true`.
- **`static/pretty-feed-v3.xsl`** — Custom XSLT stylesheet for the RSS feed, providing a styled browser-readable feed view.

### i18n

Partial translations exist for English (`en`), German (`de`), and French (`fr`) in `i18n/`. Strings used in templates (e.g., "Read more", "Posted on") reference these via `i18n` function calls.

### CSS Architecture

- **Source:** `assets/css/main.css` (TailwindCSS source)
- **Output:** none on disk. `head/css.html` resolves `css/main.css`, concatenates
  `css/extended/*.css`, and pipes the result through `css.PostCSS` inside the
  Hugo build, emitting a fingerprinted stylesheet to `public/css/`. There is no
  checked-in build artifact — `assets/css/style.css` was one, read by nothing,
  and was deleted in v0.3.0.
- **Tokens:** `tailwind.preset.js` (theme, darkMode, plugins — what consumers require)
- **Config:** `tailwind.config.js`, a thin wrapper adding only this repo's dev `content` globs
- **PostCSS:** `postcss.config.js` (autoprefixer processing)
- **JIT Compilation:** TailwindCSS scans `hugo_stats.json` for class usage (configured in `build.cachebusters`)

**Hugo integration:** The theme uses Hugo's asset pipeline with cache busting for Tailwind, PostCSS, and JS files.

### Configuration System

**Layered configs** (Hugo's multi-environment setup):
- `/config/_default/hugo.toml` - Base configuration
- `/config/production/hugo.toml` - Production overrides (analytics, GitInfo, etc.)

**Key parameter groups:**
```toml
[params]
  isAlpha = true                 # Global site banner
  showDarkToggle = true          # Dark mode toggle
  showCardLinkOverlay = false    # Card link behavior
  repository = "..."             # GitHub repo for footer links
  showHomeFeed = true            # Show paginated feed on home page
  showDate = true                # Show post dates
  showAuthor = true              # Show author on posts
  showBreadCrumbs = true         # Breadcrumb navigation
  showShareButtons = false       # Social share buttons on single pages
  showSummaryMeta = true         # Show meta on card summaries
  showReadOn = false             # "Read on" link on cards
  loadLeaflet = false            # Load Leaflet.js (self-hosted in static/leaflet/)
  navbar_fixed = false           # Sticky header
  og_image_default = "images/..."# Default OG image path
  logo_firstWord = "..."         # Logo text (first word)
  logo_lastWord = "..."          # Logo text (second word)
  logo_tagline = "..."           # Logo tagline
  homePageFeedHeader = "Latest"  # Home feed section header
  homePageFeatureHeader = "..."  # Featured section header
  articleBackgroundStyle = "..."    # TW classes for article bg
  featuredArticleBackgroundStyle = "..." # TW classes for featured article bg

[params.twClasses]               # TailwindCSS class overrides
  headerBackgroundFrameOuter = "bg-gradient-to-r ..."
  headerBackgroundFrameInner = "bg-cover h-[300px] ..."
  footerBackground = "..."       # Footer background (falls back to headerBackgroundFrameOuter)

[params.author]
  name = "..."
  email = "..."

[params.shareButtons]
  networks = ["twitter", "email", "reddit", "facebook"]
  # size, icon, buttonMargin also available

[params.footer]
  tagCloud = true                # Enable footer taxonomies

[[params.footer.taxonomies]]     # Opt-in taxonomy display
  name = "tags"
  title = "Top Tags"
  minCount = 2

[[params.alphaAlert]]            # Site-wide banners (can have multiple)
  alertType = "danger|info|warning|success"
  alertTitle = "..."
  alertMessage = "..."
  dismissable = true
  alertIconClass = "fa-solid fa-..."  # Optional Font Awesome icon
  weight = 1                     # Display order

[services.plausibleAnalytics]    # Privacy-friendly analytics
  ID = "yourdomain.com"
```

**Social links (data file, not params):**

Social icons in the footer are configured via `data/social.json`, NOT via params:
```json
{
  "main": [
    { "title": "GitHub", "name": "github", "icon": "fab fa-github", "link": "https://...", "weight": 10 }
  ]
}
```
Fields: `title`, `name`, `icon` (Font Awesome classes), `link`, `weight` (sort order), optional `rel`, `target`, `type`.

**Home page feed filtering (front matter):**

Pages with `homeFeature: true` appear in the featured grid (not the paginated feed). The feed excludes pages with empty summaries, and respects site params `excludedSections`, `excludedCategories`, `excludedTags`. Sort order is controlled via `listSortBy` and `listSortOrder` params.

## Critical Installation Notes

### When Copying exampleSite Config
**ALWAYS delete** the line `themesDir = "../.."` from `/config/_default/hugo.toml`. This is for theme development only and will break normal installations.

### GitInfo Performance Warning
- **`enableGitInfo = true`** runs git commands for every page to get commit metadata
- Used for footer GitHub links (commit/history/blame) via `layouts/partials/utils/page_meta.html`
- **Performance impact:** 40-50% slower builds on sites with 1000+ pages
- **Default:** Disabled for performance
- Only enable if GitHub footer links are specifically needed

### Footer Taxonomies (Opt-in)
By default, footer shows NO taxonomy lists. Must configure `params.footer.taxonomies` to display tags/categories.

## Development Philosophy

**Barebones with practical additions** - Avoid over-engineering:
- Don't add features beyond what's requested
- Use TailwindCSS utilities over custom CSS
- Keep Alpine.js usage minimal
- Don't add error handling for impossible scenarios
- Don't create abstractions for one-time operations
- Don't add backwards-compatibility hacks

## Development Workflow

### Adding New Features
1. Read existing code first to understand patterns
2. Update `layouts/` or `layouts/partials/` as needed
3. Run `npm test` and `hugo --source exampleSite --destination /tmp/ryder-check`
4. Document in README.md if user-facing
5. Update `theme.toml` version if significant
6. Maintain Hugo 0.146+ compatibility

### Modifying CSS
1. Edit source files in `assets/css/main.css`
2. Run `hugo server -D --source exampleSite` — Tailwind recompiles in-build
3. Design tokens (theme, darkMode, plugins) live in `tailwind.preset.js`, which
   consumers require; `tailwind.config.js` only adds this repo's `content` globs

### Creating Shortcodes
1. Add to `layouts/shortcodes/`
2. Create example in `exampleSite/content/posts/`
3. Document usage in README

### Template Customization Pattern
When creating new template variants:
- Follow the `{partial}{suffix}.html` naming pattern
- Use `.Param "{type}Type"` to load variants
- Support both config-level and front-matter overrides
- Test default behavior when parameter is unset

## CI/CD

GitHub Actions (`.github/workflows/hugo.yml`) builds exampleSite and deploys to GitHub Pages on every push to `main`. Check build badge status before merging changes.

**Workflow details:**
- Uses Hugo 0.138.0 (⚠️ lower than the 0.146+ requirement — update when bumping minimum Hugo version)
- Runs `npm ci` in exampleSite directory
- Builds with `--minify` flag
- Deploys to https://arts-link.github.io/ryder/

## Known Issues

- Theme is v0.1 (work in progress)
- Archetype documentation needs expansion
- Not all configuration variables are documented

## Design Decisions to Ask About

When implementation approach is unclear:
- Should this be a shortcode or a partial?
- Does this need configuration via hugo.toml?
- Should this support the variant suffix pattern (e.g., `-custom`)?
- Should this be demonstrated in exampleSite?
- Does this align with "barebones with practical additions"?
- Will this work across all production sites using the theme?

## Resources

- Hugo Discourse: https://discourse.gohugo.io/
- TailwindCSS Docs: https://tailwindcss.com/docs
- Alpine.js Docs: https://alpinejs.dev/
